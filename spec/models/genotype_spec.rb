...
describe Genotype do
  before :all do
    srand(1)
    @c0 = FactoryGirl.build(:genotype)
    @c1 = FactoryGirl.build(:genotype)
    @c2 = FactoryGirl.build(:genotype)
  end
  ...
  context "mutation" do
    ...
    it "should generate a random seed part" do
      srand(1)
      Genotype.rand_hex.should eq('6AC1F3')
    end
  ...
  context "reproduction" do
    ...
    it 'should be a genotype' do
      @c3.should be_a(Genotype)
    end
